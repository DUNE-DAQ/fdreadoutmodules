...
#define TRACE_NAME "CRTdecode"
#define TLVL_CRTDECODE 16
                          ...
                          uint32_t in24bitword)
{
  ...
  {
    //TLOG(TLVL_DEBUG, "CRTdecode") 
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Got a 24-bit word that starts with " 
	      << ((in24bitword >> 22) & 3) << "\n";
    return false;
  }
  ...
  {
    //TLOG(TLVL_DEBUG+1, "CRTdecode") 
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Throwing out a word that is not a UNIX time word or hit data: " << (in24bitword << 16) << "\n";
    return false;
  }
                       ...
                       const unsigned int max_cooked)
{
  ...
  if(size_needed > max_cooked){
    //TLOG(TLVL_WARNING, "CRTdecode") 
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Can't write packet of size " << size_needed << " with max " << max_cooked << "\n";
    return 0;
  }
  ...
  if(bytes != size_needed)
    //TLOG(TLVL_WARNING, "CRTdecode") 
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: size mismatch " << bytes << " != " << size_needed << "\n";
  return bytes;
}
                           ...
                           const int baselines[maxModules][numChannels])
{
  ...
  while(!raw.empty() && raw[0] != 0xffff){
    //TLOG(TLVL_WARNING, "CRTdecode")
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecoe: Discarding word 0x" << raw[0] << " appearing before 0xffff\n";
    raw.pop_front();
  }
  ...
  if(len == 0){
    //TLOG(TLVL_WARNING, "CRT")
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Discarding packet with declared length zero.\n";
    raw.clear();
    return 0;
  }
  ...
  if(!(raw[ADC_WIDX_MODLEN] >> 15)){
    //TLOG(TLVL_WARNING, "CRT")
    TLOG_DEBUG(TLVL_CRTDECODE) << "Non-ADC packet found, skipping\n";
    ...
    if(wordi == ADC_WIDX_CLKHI) {
      ...
      if(wordi%2 == 0) {
        ...
        if(hit.channel < numChannels && packet.module < maxModules)
          ...
          //TLOG(TLVL_DEBUG, "CRT")
          TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Not applying baseline to bad module/channel number " 
  ...
  if(!goodparity){
    //TLOG(TLVL_WARNING, "CRT")
    TLOG_DEBUG(TLVL_CRTDECODE) << "Parity error.  Dropping packet.\n";
    return 0;
  }
 ...
 */
  ...
  for(char * readptr = rawfromhardware; readptr < next_raw_byte; readptr++){
    ...
    if(counter == 0){
      ...
      if(++expcounter == 4){ //TODO: If we don't get here, we never try to make a packet
        ...
        //TLOG(TLVL_DEBUG+1, "CRTdecode")
        TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Throwing out word " << word << " in raw2cook() because "
                  << (raw24bit_success?"make_a_packet()":"raw24bit_to_raw16bit()") << " failed\n";
      }
    }
    else{
      //TLOG(TLVL_WARNING, "CRT")
      TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: corrupted data: expected counter " << expcounter 
	        << ", got " << counter << "\n";
      expcounter = 0;
    }
  }
  ...
  if(used_raw_bytes){
    //TLOG(TLVL_DEBUG+1, "CRT")
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Used " << used_raw_bytes << "bytes, and rotating " 
      << next_raw_byte - rawfromhardware - used_raw_bytes << " to front for later use.\n";
    memmove(rawfromhardware, rawfromhardware + used_raw_bytes,
            next_raw_byte - rawfromhardware - used_raw_bytes);
  }
  else{
    //TLOG(TLVL_DEBUG+1, "CRTdecode")
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Used 0 bytes\n";
  }
  ...
  if(tpacket == 1){
    //TLOG(TLVL_DEBUG, "CRTdecode") 
    TLOG_DEBUG(TLVL_CRTDECODE) << "CRTdecode: Tpacket error: " << tpacket <<"\n";
  }
  return cooked_bytes;
}
