...
CRTInterface::CRTInterface(const std::string indirec, const unsigned int usbnum) :
...
void CRTInterface::StartDatataking()
{
  ...
  std::cout << "CRTInterface: starting data-taking\n"; 
  if(inotifyfd != -1){
    //TLOG(TLVL_WARNING, "CRTInterface")
    std::cout << "inotify already init'd. Ok if we stopped and restarted data taking.\n";
    return;
  }
  ...
  if(-1 == (inotifyfd = inotify_init())){
    ...
    std::cout << "CRTInterface::StartDatataking: " << strerror(errno);
  }
                         ...
                         const unsigned int usbnumber)
{
  ...
  if((dp = opendir(indir.c_str())) == NULL){
    if(errno == ENOENT){
      //TLOG(TLVL_WARNING, "CRTInterface") 
      std::cout << "No such directory " << indir << ", but will wait for it\n";
      usleep(100000);
      return NULL;
    }
    else{
      ...
      //throw cet::exception("CRTInterface")
      std::cout  << "find_wr_file opendir: " << strerror(errno);
    }
  }
  ...
  if(errno){
    //throw cet::exception("CRTInterface")
    std::cout << "find_wr_file readdir: " << strerror(errno);
  }
  errno = 0;
  closedir(dp);
  if(errno){
    //throw cet::exception("CRTInterface")
    std::cout  << "find_wr_file closedir: " << strerror(errno);
  }
...
bool CRTInterface::try_open_file()
{
  ...
  if(cplusplusiscrazy == datafile_name){
    ...
    TLOG_DEBUG(TLVL_CRTINTERFACE) << "Looked for a new file from the backend, but found the current file instead!  "
  ...
  if(strlen(filename) == 0){
    //TLOG(TLVL_DEBUG, "CRTInterface") 
    std::cout << "Failed to find input file " << filename << " in directory " 
    << indir << "/binary/ for raw data.\n";
    return false;
  }
  else{
    //TLOG(TLVL_DEBUG, "CRTInterface")
  ...
  close(datafile_fd);
  //TLOG(TLVL_DEBUG, "CRTInterface")
   std::cout << "Closed file " << datafile_name << " and "
  ...
  if(-1 == (inotify_watchfd =
    ...
    if(errno == ENOENT){
      ...
      //TLOG(TLVL_INFO, "CRTInterface")
      std::cout << "File has vanished. We'll wait for another\n";
      return false;
    }
    else{
      // But other inotify_add_watch errors we probably can't recover from
      //throw cet::exception("CRTInterface") 
      std::cout << "CRTInterface: Could not open "
  ...
  //TLOG(TLVL_INFO, "CRTInterface") 
  std::cout << "Just opened and now reading from file " << datafile_name 
            << ".  Found it at time " << time(nullptr) << "\n";
...
bool CRTInterface::check_events()
{
  ...
  if(-1 ==
    ...
    //throw cet::exception("CRTInterface")
    std::cout << "CRTInterface::FillBuffer: " << strerror(errno);
  }
  ...
  if(inotify_bread < (ssize_t)sizeof(struct inotify_event)){
    //throw cet::exception("CRTInterface") 
    std::cout  << "Non-zero, yet wrong number (" << inotify_bread << ") of bytes from inotify\n";
  }
...
size_t CRTInterface::read_everything_from_file(char * cooked_data)
{
  ...
  if(read_bread == -1){
    ...
    //throw cet::exception("CRTInterface")
    std::cout << "CRTInterface::read_everything_from_file: " << strerror(errno);
  }

  const int bytesleft = next_raw_byte - rawfromhardware;
  //TLOG(TLVL_DEBUG, "CRTInterface")
  TLOG_DEBUG(TLVL_CRTINTERFACE) << bytesleft << " bytes in raw buffer after read.\n";

  if(bytesleft > 0) state |= CRT_DRAIN_BUFFER;

  return CRT::raw2cook(cooked_data+16, COOKEDBUFSIZE,
...
{
  //TLOG(TLVL_DEBUG, "CRTInterface")
  //std::cout  << "Entering CRTInterface::FillBuffer";
...
std::memcpy(cooked_data + 8, &daqheader2, 8);

/////////////////////////////////////////
  static const char zchar = '\0';    
  *bytes_ret = 0;
  ...
  if(state & CRT_DRAIN_BUFFER){
    if((*bytes_ret = CRT::raw2cook(cooked_data+16, COOKEDBUFSIZE,
                                   rawfromhardware, next_raw_byte, baselines, tpacket)))
      {
        //TLOG(TLVL_WARNING, "CRTInterface") << "Tpacket is " << tpacket << "\n";
        
	//////Fill to a fixed size with zeroes//////////
	for(size_t j = *(bytes_ret)+16; j<288; j++){
  ...
  if(state & CRT_READ_MORE){
    state &= ~CRT_READ_MORE;
    //TLOG(TLVL_DEBUG, "CRTInterface")
    TLOG_DEBUG(TLVL_CRTINTERFACE) << "Read data from " << datafile_name
  ...
  if(state & CRT_WAIT){
    ...
    if(oldState & CRT_FIRST_FILE)
    {
      ...
      //TLOG(TLVL_INFO, "CRTInterface")
      std::cout << "First input file, skipping to end: " << datafile_name << "\n";
    ...
    //TLOG(TLVL_DEBUG, "CRTInterface") 
    std::cout << "Read data from " << datafile_name
              << " because a new file was found.  ";
    //TLOG(TLVL_DEBUG, "CRTInterface")
    std::cout << "Buffer had " << bytesBefore 
...
void CRTInterface::SetBaselines()
{
  ...
  std::cout << "CRTInterface: Entering SetBaselines()\n";
  ...
  while(true){
	...
	std::cout << "Waiting for baseline file to appear at " << indir << "\n";
        ...
        //throw cet::exception("CRTInterface") 
        std::cout << "Can't open baseline file: " << strerror(errno) << "\n";
      }
    }
    else{
  ...
  std::cout << "CRTInterface: Baseline file opened: " << (indir + "/baselines.dat") << std::endl;
  int usb = 0, module = 0, channel = 0;
  float fbaseline = 0, stddev = 0, nhit = 0;
  int nconverted = 0;
  int line = 0;
  //std::cout << "CRTInterface test: module = " << module << std::endl;
  while(EOF != (nconverted = fscanf(in, "%d,%d,%d,%f,%f,%f",
    ...
    if(nconverted != 6){
      //TLOG(TLVL_WARNING, "CRTInterface")
      std::cout << "Warning: skipping invalid line "
    ...
    if(module >= 64){
      //TLOG(TLVL_WARNING, "CRTInterface")
         std::cout << "Warning: skipping baseline with invalid module number "
    ...
    if(channel >= 64){
      //TLOG(TLVL_WARNING, "CRTInterface")
      std::cout << "Warning: skipping baseline with invalid channel number "
         << module << ".  Valid range is 0-63\n";
      continue;
    }
    ...
    if(nhit < 100)
      //TLOG(TLVL_WARNING, "CRTInterface")
       std::cout << "Warning: using baseline based on only " << nhit << " hits (PMT " << module << ", Ch. " << channel << ")\n";

    if(stddev > 7.0)
      //TLOG(TLVL_WARNING, "CRTInterface")
       std::cout << "Warning: using baseline with large error: " << stddev << " ADC counts (PMT " << module << ", Ch. " << channel << ")\n"; 
  ...
  if(fclose(in) == EOF){
    //throw cet::exception("CRTInterface")
    std::cout << "Can't close CRT baseline file: " << strerror(errno) << "\n";
  }
  std::cout << "CRTInterface: Exiting SetBaselines()\n";
}
